...
-- Based on work by: Alessandro Apostoli, https://github.com/yaapu
--
...
function checkEvents()
  ...
  if status.showMinMaxValues == false then
    ...
    -- HDOP quality alert: warn when GPS HDOP exceeds configured threshold (added by Arya MGC)
    if status.conf.maxHdopAlert > 0 and status.telemetry.gpsStatus > 2 then
      libs.utils.checkAlarm(status.conf.maxHdopAlert*100, status.telemetry.gpsHdopC, 12, 1, "hdop", status.conf.repeatAlertsPeriod)
    end
...
local function createOnce(widget)
  ...
  libs.utils.pushMessage(7, "Yaapu Telemetry Widget 1.6.0-arya".. " ("..'c137d01'..")")
...
local function configure(widget)
  ...
  form.addStaticText(line, nil, "1.6.0-arya".." ("..'c137d01'..")")
